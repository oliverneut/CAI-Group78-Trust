...
class LyingAgent(BaseLineAgent):
        ...
        while True:
            ...
            if Phase.DROP_BLOCK == self._phase:
                ...
                if random.random() > 0.8:  # Tell the truth
                    self._sendMessage('Dropped goal block ' + self.visualize(
                        self._holdingBlock['visualization']) + ' at drop location ' + str(self._destination),
                                      self._agentName)
                else:  # Lie
                    self._sendMessage('Dropped goal block ' + self.visualize(
                        self.randomGoalblock(self._holdingBlock)['visualization']) + ' at drop location ' + str(self._destination),
                                      self._agentName)
    ...
    def searchRoom(self, state: State):
        if random.random() > 0.8:  # Tell the truth
            ...
            self._sendMessage('Searching through ' + self.randomRoom(state, self._door['room_name'])['room_name'], self._agentName)
        ...
        if foundBlock:
            if random.random() > 0.8:  # Tell the truth
                self._sendMessage(
                    'Picking up goal block ' + self.visualize(vBlock['visualization']) + ' at ' + str(
                        vBlock['location']),
                    self._agentName)
            else:  # Lie
                self._sendMessage(
                    'Picking up goal block ' + self.visualize(self.randomGoalblock(vBlock)['visualization']) + ' at ' + str(
                        vBlock['location']),
                    self._agentName)
    ...
    def planPathToClosedDoor(self, state: State):
        ...
        if random.random() > 0.8:  # Tell the truth
            ...
            self._sendMessage('Moving to ' + self.randomRoom(state, self._door['room_name'])['room_name'], self._agentName)
        ...
        for i in range(len(self._goalBlocks)):
            ...
            if self._retrievedGBlocks[i] != 1:
                ...
                for vBlock in self._visibleBlocks:
                    ...
                    if self.visualize(vb) == self.visualize(gb):
                        if random.random() > 0.8:  # Tell the truth
                            msg = 'Found goal block ' + self.visualize(vb) + ' at location ' + str(vBlock['location'])
                        else:  # Lie
                            msg = 'Found goal block ' + self.visualize(self.randomGoalblock(vBlock)['visualization']) + ' at location ' + str(vBlock['location'])
    ...
    def randomRoom(self, state, notThisRoom):
        doors = [door for door in state.values() if 'class_inheritance' in door and 'Door' in door['class_inheritance'] if door['room_name'] != notThisRoom]
        return random.choice(doors)

    def randomGoalblock(self, notThisBlock):
        blocks = [block for block in self._goalBlocks if self.visualize(block['visualization']) != self.visualize(notThisBlock['visualization'])]
